...
def tokenize_plain_string_and_extract_label_seq(string: str, tag2label: Dict[str, str]={'D2': 'D', '?': 'W'}) -> Tuple[List[str], List[str]]:
    """カナ表記の文字列（タグ付き）をカナ記号列とラベル列に変換する.

    実行例:
        >>> tokenize_plain_string_and_extract_label_seq('(D2 これ(F エー)何ですか(? あ,の))や(F えー)はい')
        ['こ', 'れ', 'え',   'ー',  '何', 'で', 'す', 'か', 'あ', 'の', 'や', 'え', 'ー', 'は', 'い'],
        ['ND', 'ND', 'NDF', 'NDF', 'D',  'D', 'D',  'D', 'ND?', 'ND?', 'N',  'NF', 'NF', 'N', 'N']
        ...
        if match:
            ...
            while len(content) > 0:
                if content.startswith('<sp>'):
                    ...
                    char_list.append(content[0])
                    content = content[1:]

            result.extend(char_list)
            result_label.extend(['N'] * len(char_list))
            ...
            if tag in ('F', 'D', 'D2', 'M', 'O', 'X', '笑', '泣', '咳', 'L'):
                ...
                if tag == '?':
                    ...
                    raise Exception('Unknown tag: {} in {}'.format(tag, tagged_string))
            ...
            # content がタグを含む場合は再帰処理をする
            if '(' in content:
                char_list, tag_list = tokenize_plain_string_and_extract_label_seq(content, tag2label)
                tag_list = [t + label_tag for t in tag_list]
            else:
                char_list = []
                while len(content) > 0:
                ...
                tag_list = ['N'+label_tag] * len(char_list)
            result.extend(char_list)
            result_label.extend(tag_list)
...
def make_text_tokenized_plain(flattened_trn,
                             remove_word_sep=False, remove_sp=False,
                             remove_privacy_utt=True,
                             remove_extra_content=True):
    """平文の文字ベースのスペース区切りのリスト，およびパラ言語情報のスペース区切りリストを作成する
    ...
    for utterance in flattened_trn['flattened_utterances']:
        ...
        # print(utt_id, text)
        char_list, label_list = tokenize_plain_string_and_extract_label_seq(text)

        texts.append(f"{utt_id} {' '.join(char_list)}")
        labels.append(f"{utt_id} {' '.join(label_list)}")
        segments.append(f"{utt_id} {wav_id[utterance['channel']]} {utterance['start']:.3f} {utterance['end']:.3f}")
    ...
    for t, l in zip(texts, labels):
        ...
        for t_token, l_token in zip(tbody.split(' '), lbody.split(' ')):
            for para_key in para_keys:
                if t_token != "<sp>" and para_key in l_token:
...
def main(trn_file_path, output_dir, mode='tokenized_kana', wav_file_path=None,
    ...
    elif mode == 'tokenized_plain' or mode == 'tokenized_plain_para':
        texts, labels, segments = make_text_tokenized_plain(flattened)
    elif mode == 'kana':
    ...
    if mode == 'tokenized_plain_para' or mode == 'tokenized_kana_para':
...
if __name__ == "__main__":
    ...
    parser.add_argument('mode', choices=['plain', 'tokenized_plain', 'tokenized_plain_para', 'kana', 'tokenized_kana', 'tokenized_kana_para'], help='Output mode')
